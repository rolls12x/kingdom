...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.id === msg.author.id)
    return msg.reply("Kendiniz ile oyun oynayamazsınız.");
  if (this.games.has(msg.channel.id))
    return msg.reply("Kanal başına sadece bir düello meydana gelebilir.");
  ...
  try {
    ...
    return msg.channel.send(
      winner ? `Tebrikler, ${winner}!  ` : "Oh... Hiçkimse kazanamadı."
    );
  } catch (err) {
    this.games.delete(msg.channel.id);
    throw err;
